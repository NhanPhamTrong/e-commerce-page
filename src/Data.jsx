...
const AddRandom = (data) => {
    return data.map(d => {
        const randomCurrentPrice = Math.floor(Math.random() * 200) / 100 + 100
        const randomPreviousPrice = randomCurrentPrice + Math.floor(Math.random() * 20) / 100 + 10
        const numberOfRating = Math.floor(Math.random() * 200) + 50
        const numberOfSold = numberOfRating + Math.floor(Math.random() * 200) + 20
        return ({
            ...d,
            stars: Math.floor(Math.random() * 2) / 10 + 3,
            numberOfRating: numberOfRating,
            sold: numberOfSold,
            previousPrice: Math.floor(Math.random() * 100) > 75 ? randomPreviousPrice : 0,
...
// get(child(dbRef, "doesHadData")).then(snapshot => {
//     if (snapshot.exists()) {
//         if (snapshot.val() === 0) {
//             set(child(dbRef, "productList"), AddRandom(initialData))
//             set(child(dbRef, "doesHadData"), 1)
//         }
//     } else {
//         set(child(dbRef, "doesHadData"), 0)
//     }
// }).catch((error) => {
//     console.error(error)
// })
